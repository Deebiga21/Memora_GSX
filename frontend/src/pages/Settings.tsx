...
export default function Settings() {
  ...
  const Toggle = ({ label, desc, settingKey }: { label: string, desc: string, settingKey: string }) => (
    <div className="flex items-center justify-between py-2">
      <div>
        <div className="font-bold text-[#F4EFE6] text-sm">{label}</div>
        <div className="text-xs text-[#A18A68] font-medium">{desc}</div>
      </div>
      <div 
        onClick={() => handleToggle(settingKey)}
        className={`w-10 h-6 rounded-full relative cursor-pointer transition-colors ${settings[settingKey] ? 'bg-[#DFCEB6]' : 'bg-[#3D3A35]'}`}
      >
        <div className={`w-4 h-4 bg-[#2C2A28] rounded-full absolute top-1 transition-all ${settings[settingKey] ? 'right-1' : 'left-1'}`}></div>
      </div>
    </div>
  );
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 pb-12">
      <div className="border-b border-[#5A544A] pb-6 flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-[#F4EFE6] mb-1">Settings</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
         <div className="space-y-1">
            <button className="w-full flex items-center gap-3 px-4 py-2.5 text-sm font-bold bg-[#DFCEB6] text-[#2C2A28] rounded-lg transition-colors border border-[#83633F]">
         ...
         <div className="md:col-span-3 space-y-8">
            {/* AI Assistant */}
            <div className="bg-[#201D19] border border-[#5A544A] rounded-xl shadow-sm overflow-hidden">
               <div className="px-6 py-4 border-b border-[#3D3A35] bg-[#2C2A28]">
                  <h3 className="font-bold text-[#F4EFE6] text-sm uppercase tracking-wider">AI Assistant</h3>
               </div>
               <div className="p-6 space-y-4">
                  <Toggle label="Voice Input" desc="Allow speaking questions via microphone" settingKey="voice_input" />
                  <div className="h-px bg-[#3D3A35] w-full"></div>
            ...
            <div className="bg-[#201D19] border border-[#5A544A] rounded-xl shadow-sm overflow-hidden">
               <div className="px-6 py-4 border-b border-[#3D3A35] bg-[#2C2A28]">
                  <h3 className="font-bold text-[#F4EFE6] text-sm uppercase tracking-wider">Notifications</h3>
               </div>
               <div className="p-6 space-y-4">
            ...
            <div className="pt-6 border-t border-[#5A544A]">
               <Link to="/" className="inline-flex items-center gap-2 px-4 py-2 text-red-500 hover:bg-[#2C2A28] rounded-lg text-sm font-bold transition-colors">
